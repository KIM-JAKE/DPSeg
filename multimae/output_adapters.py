...
class SegmenterMaskTransformerAdapter(nn.Module):
    ...
    def __init__(
        ...
        self.in_channels = 768 * len(self.main_tasks)
...
class MaskFormer(nn.Module):
    ...
    def forward(self, encoder_tokens: torch.Tensor, input_info: Dict):
        ...
        mask = x.permute(0, 2, 1) 
        mask = mask.view(1, 768, 40, 40) 
        mask = self.final_layer2(mask)
        mask = mask.view(1, 40, 1600) 
        binary_mask = (mask > 0.2).float()  # 임계값 0.5를 기준으로 이진화     
        token = self.token_blocks(cls_emb , x, mask = binary_mask) # 40, 768
